...
func TestParseSpec_ReturnsOperations(t *testing.T) {
	ops, err := ParseSpec("../spec/confluence-v2.json")
	if err != nil {
...
func TestParseSpec_GetPage(t *testing.T) {
	ops, err := ParseSpec("../spec/confluence-v2.json")
	if err != nil {
		t.Fatalf("ParseSpec error: %v", err)
	}

	var getPage *Operation
	for i := range ops {
		if ops[i].OperationID == "getPageById" {
			getPage = &ops[i]
			break
		}
	}
	...
	if getPage == nil {
		t.Fatal("getPageById operation not found")
	}

	if getPage.Method != "GET" {
		t.Errorf("expected method GET, got %s", getPage.Method)
	}

	if len(getPage.PathParams) == 0 {
		t.Error("expected path params for getPageById, got none")
	}
}
