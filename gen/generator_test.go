...
func TestBuildPathTemplateMalformedBrace(t *testing.T) {
	...
	tmpl, args := buildPathTemplate("/pages/{broken")
...
func TestBuildPathTemplateEmptyParam(t *testing.T) {
	...
	tmpl, args := buildPathTemplate("/pages/{}/items")
	if !strings.Contains(tmpl, "{") {
	...
	groups := map[string][]Operation{
		"pages": {
			{OperationID: "getPageById", Method: "GET", Path: "/pages/{id}",
				PathParams:  []Param{{Name: "id", In: "path", Required: true, Type: "string"}},
				QueryParams: []Param{{Name: "body-format", In: "query", Required: false, Type: "string", Description: "body format"}},
				HasBody:     false},
			{OperationID: "createPage", Method: "POST", Path: "/pages", HasBody: true},
		},
	}

	if err := GenerateSchemaData(groups, []string{"pages"}, dir); err != nil {
...
func TestGenerateResource(t *testing.T) {
	ops := []Operation{
		{
			...
			PathParams: []Param{
				{Name: "id", In: "path", Required: true, Type: "string", Description: "The page ID"},
			},
			QueryParams: []Param{
				{Name: "body-format", In: "query", Required: false, Type: "string", Description: "Body format"},
				{Name: "get-draft", In: "query", Required: false, Type: "boolean", Description: "Get draft version"},
			},
			HasBody: false,
		},
		{
	...
	dir := t.TempDir()
	if err := GenerateResource("pages", ops, dir); err != nil {
		t.Fatalf("GenerateResource: %v", err)
	}

	content, err := os.ReadFile(filepath.Join(dir, "pages.go"))
	if err != nil {
	...
	checks := []string{
		...
		"DO NOT EDIT",
		`Use:   "pages"`,
		`Use:   "get-by-id"`, // DeriveVerb("getPageById", resource="pages") → strips "Page" prefix → "get-by-id"
		`Use:   "create"`,
		`id`,
		`"body-format"`,
		`"get-draft"`,
	...
	for _, want := range checks {
		if !strings.Contains(src, want) {
			t.Errorf("generated pages.go missing %q", want)
		}
	}
}
