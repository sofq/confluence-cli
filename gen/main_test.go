...
func TestRun(t *testing.T) {
	outDir := t.TempDir()
	err := run("../spec/confluence-v2.json", outDir)
	if err != nil {
		t.Fatalf("run: %v", err)
	}

	// Verify output files exist.
	for _, name := range []string{"init.go", "schema_data.go", "pages.go"} {
...
func TestRunGenerateResourceError(t *testing.T) {
	...
	err := run("../spec/confluence-v2.json", t.TempDir())
	if err == nil {
...
func TestMainExitSuccess(t *testing.T) {
	...
	// Copy the spec file.
	specData, err := os.ReadFile("../spec/confluence-v2.json")
	if err != nil {
		t.Fatalf("ReadFile: %v", err)
	}
	if err := os.WriteFile(filepath.Join(specDir, "confluence-v2.json"), specData, 0o644); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}
...
func TestMainError(t *testing.T) {
	// Run main() from a temp dir where spec/confluence-v2.json doesn't exist.
	origDir, err := os.Getwd()
	if err != nil {
		t.Fatalf("Getwd: %v", err)
	}
