...
func TestNewEncoder(t *testing.T) {
	...
	s := buf.String()
	if strings.Contains(s, `\u0026`) {
		t.Error("NewEncoder should not escape & to \\u0026")
	}
	if !strings.Contains(s, "&") {
		t.Error("NewEncoder output should contain literal &")
	}
}

func TestNewEncoderNoEscape(t *testing.T) {
	var buf bytes.Buffer
	enc := NewEncoder(&buf)
	...
	if !strings.Contains(s, "<script>") {
		t.Error("NewEncoder output should contain literal <script>")
	}
	if strings.Contains(s, `\u003c`) {
		t.Error("NewEncoder should not escape < to \\u003c")
	}
