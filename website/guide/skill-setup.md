...
| Capability | How |
|-----------|-----|
| **Discover commands** | `cf schema` for runtime discovery --- no hardcoded knowledge |
| **Workflow commands** | `cf workflow` for move, copy, publish, archive, comment, restrict --- no raw JSON |
| **Minimize tokens** | `--preset` for common field sets, `--fields` + `--jq` for custom filtering |
| **Batch calls** | `cf batch` for multiple operations in one process |
| **Handle errors** | Branch on exit codes: 0=ok, 2=auth, 3=not_found, 5=rate_limited |
| **Configure auth** | Setup with `cf configure`, profiles, env vars |
